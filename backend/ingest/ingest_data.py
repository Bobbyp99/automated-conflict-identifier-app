...
def ingest_official_data(sheets):
    ...
    for sheet_name, sheet_data in sheets.items():
        ...
        new_df["name_suffix"] = ""
        new_df["jurisdiction_id"] = 1 # this is just a placeholder, will need to be updated to reflect actual jurisdiction id from jurisdictions table
        if new_df is not None:
                all_jurisdictions.append(new_df)
    
    all_jurisdictions_df = pd.concat(all_jurisdictions, ignore_index=True)
    all_jurisdictions_df = all_jurisdictions_df.drop_duplicates(subset=['cleaned_name'])
    
    save_to_postgres(all_jurisdictions_df, "officials")
